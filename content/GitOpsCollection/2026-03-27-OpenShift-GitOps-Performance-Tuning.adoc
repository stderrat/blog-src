...
For example, imagine you have a single repositories with thousands of commits. Argo CD synchronises per default the whole repositories including the history. This is one screw that can be tightened.

NOTE: In this article we will performa the configuration for OpenShift-GitOps. The settings are done by configuring the ArgoCD custom resource. If you use plain Argo CD without the operator, you can achieve the same goal by configuring the Argo CD configuration (ConfigMap).
...
When starting with this article, I did not expect to fall into the rabbit hole. The article became bigger and bigger and I am pretty sure that there will be settings that I missed, even though I search the actual source code. 

NOTE: If you find anything missing, please let me know. 

The following overview table summarises the settings discussed in this article: **default** and **example recommended** values are indicative—always validate in your environment.

// Five columns: four text cells, then one `a|` cell with YAML for **OpenShift GitOps** (`openshift-gitops` namespace).
// Widths: narrow Default/Recommended; wide OpenShift YAML. Column 1 wrapping is styled in `static/css/settings-overview.css` (`custom_css` in site config).
[width="100%",cols="22%,6%,6%,22%,44%",options="header",frame=all,grid=all,role=settings-overview]
|===
...
| <<setting-controller-repo-server-timeout,ARGOCD_APPLICATION_CONTROLLER_REPO_SERVER_TIMEOUT_SECONDS>> +
...
| <<setting-kubectl-parallelism,ARGOCD_APPLICATION_CONTROLLER_KUBECTL_PARALLELISM_LIMIT>> +
...
CAUTION: When you work with *public* repositories you might not have this Secret object. In that case you need to create it for the public repository.
...
This setting complements `ARGOCD_EXEC_TIMEOUT`. It is used to **forcibly terminate a child process** (such as Helm, Kustomize, plugins, or Git) if it cannot exit gracefully.
...
This variable sets the **maximum number of retry attempts** for failed Git repository requests. When Argo CD interacts with your Git provider, network hiccups, temporary outages, or rate limiting and throttling by the provider can cause a Git request to fail unexpectedly.

**Problem**

For example, if multiple concurrent requests hit the provider, the provider might randomly reset the connection (often seen as a connection reset by peer or ssh: handshake failed error in the logs). With the default setting of 1 attempt, Argo CD will immediately fail the sync or reconciliation process when this happens, turning your application status Unknown or throwing a sync error.
...
When the repo server downloads a Git repository and generates Kubernetes manifests, doing so is computationally expensive.
...
Increasing this lock timeout gives the first worker longer to finish rendering and populate the cache. Other workers stay in the queue instead of duplicating the work.
...
However, this API server timeout defaults to 60 seconds, so the UI can give up, drop the connection, and show a red **context deadline exceeded** error even though the repo server is still rendering in the background.
...
NOTE: Implicitly, this means that sharding only makes sense if you have more than one cluster in your environment.
...
The following algorithms are supported. Configure them with the `ARGOCD_CONTROLLER_SHARDING_ALGORITHM` environment variable when you use the OpenShift GitOps Operator, or with `controller.sharding.algorithm` in `argocd-cmd-params-cm`:
...
If these settings are not set, the controller will use the default values. This differs from the section above, where such parameters were set at `spec.extraConfig`.
...
I am sure there is a design decision behind this ... but I am not sure what it is.
...
ConfigMap: **argocd-cmd-params-cm** (if not using the Operator)
ArgoCD: spec.controller.env (if not using the openshift-gitops Operator)
