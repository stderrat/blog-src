...
featured_image: /openshift-platform/networking/images/gateway-api/gateway-api.png
...
aliases: [
   ...
   "/openshift-platform/day-2/networking/2025-08-31-openshift-gateway-api-ii/",
]
---

:imagesdir: /openshift-platform/networking/images/gateway-api/
:icons: font
:toc:
...
WARNING: b2cc commented that deploying the gateway in the
_openshift-ingress_ namespace, as we did below, is not support by
Red Hat. We are not aware this is documented somewhere (please let use
now if it is). Only after opening a support ticket he learning that
