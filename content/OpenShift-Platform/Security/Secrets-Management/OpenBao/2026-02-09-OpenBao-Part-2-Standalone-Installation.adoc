...
sudo openssl req -new -key /etc/openbao.d/tls/tls.key \
  -out /etc/openbao.d/tls/tls.csr \
  -subj "/CN=openbao.example.com"

# Generate self-signed certificate
sudo openssl x509 -req -days 365 \
  -in /etc/openbao.d/tls/tls.csr \
  -signkey /etc/openbao.d/tls/tls.key \
  -out /etc/openbao.d/tls/tls.crt
