...
. First Preare the TLS directory if it does not exist
+
[source,bash]
----
...
EOF
----
<1> The IP address of the server. In this case we are using the localhost, but you can add your IPs here.
...
Before we use  OpenBao we need to initialize it. This will create the unseal keys and the root token. The unseal keys ares used to ... well unseal the OpenBao service. 
The root key is used as a master key to authenticate against the OpenBao service. This key has access to ALL secrets. **Treat this root key with care**

We will initialize OpenBao with default options of5 key shares and 3 threshold. This means that we need 3 different unseal keys to unseal the OpenBao service.
...
Thats it for the basic verification. You can now start to use OpenBao in your production environment. Let's see what we can do in the UI.

== Visiting the UI

If you watached the previous commands closely you will have noticed that the UI has been enabled in the configuration file: 
...
We have created th path "secret" and inside a secret called "test". 
Which we can retrieve now: 
