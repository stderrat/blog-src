...
featured_image: /openshift-platform/infrastructure/images/okd-sno-disconnected/okd_disconnected_cat.webp
...
Because of reasons we had to set up a disconnected single node OKD "cluster". This is our brain dump as the
...
NOTE: Updated on 2026-03-03: Fixed install-config.yaml and create manifest command

<!--more-->

First we need to download the _oc_ command in the version we would like to install. OKD provides it on its release page:
...
For mirroring all required OKD images to our private registry we created an _ImageSetConfiguration_:
...
The hard part for us was finding the right values for _channels_ and
...
This will create the _ImageDigestMirrorSource_ in _/workspace/working-dir/cluster-resources/idms-oc-mirror.yaml_ and _ImageTagMirrorSource_ custom resources in _/workspace/working-dir/cluster-resources/itms-oc-mirror.yaml_.

The content of _ImageDigestMirrorSource_ will be reused in
...
We are now ready to download the installation ISO and to embed our ignition config into it. We also want
