...
NOTE: Updated on 2026-03-03: Fixed install-config.yaml
...
----
<1> We set the number of worker to zero (0). Because this is a SNO installation. Workers nodes could be added later, even for a SNO cluster
<2> We set the number of master nodes to one (1)
<3> This tells the installer that we do not have a bootstrap node and it should create on large ignition config file
...
We can take a look at the ISO bootstrap config and the kernels arguments with:
