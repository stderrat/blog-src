...
Our private registry is based on https://goharbor.io[Harbor] for our experiments, but any Docker v2 compatible registry should work. Then only thing required is a project
within Harbor called _openshift_ and a user/password for pulling/pushing images to this project. The project name might
be configurable with _oc mirror_, but this requires further investigation.

_oc mirror_ wants to verify signatures so we had to download a public key and provide the _OCP_SIGNATURE_URL_:
