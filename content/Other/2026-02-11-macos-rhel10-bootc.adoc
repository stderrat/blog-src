...
We followed the
https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/using_image_mode_for_rhel_to_build_deploy_and_manage_operating_systems/creating-bootc-compatible-base-disk-images-by-using-bootc-image-builder[RHEL documentation] for created a bootable qcow image from a bootc
container image.
...
that our podman machine runs rootful. Otherwise image builder will not work. So lets do this:
...
----------
<1> We had to add this dir into the container, maybe this is required because we run podman in a VM on macos (podman machine)
