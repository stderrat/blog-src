...
While noteSelector provides a very easy way to control where a pod shall be scheduled, the affinity/anti-affinity features, expands this configuration which more expressive rules like logical AND operator, constraints against lables on other pods or soft rules instead of hard requirements. 
...
* node affinity - allows pods to specify a group of nodes it can be placed on

<!--more--> 

== Pod Affinity and Anti-Affinity

*Affinity* and *Anti-Affinity* control the nodes on which a pod should (or should not) be scheduled based on labels on Pods that are already scheduled on the node. 
...
* Pod Anti-Affinity - prevents the scheduler to put a new pod onto the same nodes with pods with the same labels
+
For example: 
+ 
...
As second step the deployment configuration of the webapplication must be modified. Remember, we want that the frontend is running only on nodes, which are part of the kubernetes-zone where the postgres pod is running. In our example this would be either compute-0 or compute-1. 
...
For now the database pod and the web application pod are running on nodes of the same zone. However, somebody is asking us to configure it vice versa: the web application should not run in the same zone. 
...
This will force the django pods to run only on "west" zone nodes. 
...
. _Except for the above cases, the `topologyKey` can be any legal label-key._
